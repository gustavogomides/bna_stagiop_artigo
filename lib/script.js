...
async function criarNotificacao(NS, factory, notificacaoInfo) {
    ...
    let notificacoes = await notificacaoRegistry.getAll()

    let newId = notificacoes.length + 1
    if (notificacoes.length != 0) {
        newId = Math.max.apply(Math, notificacoes.map(function (o) { return o.notificacaoId })) + 1
    }

    var notificacao = factory.newResource(NS, 'Notificacao', newId.toString())
