...
function exameIniciadoTransaction(exameIniciado) {
    return atualizarStatusExame(exameIniciado, 'EM_PROCESSAMENTO', 'IniciarExameEvent')
}
...
function exameFinalizadoTransaction(exameFinalizado) {
    return atualizarStatusExame(exameFinalizado, 'PROCESSADO', 'FinalizarExameEvent', true)
}
...
function iniciarConsultaTransaction(consultaPendente) {
    return atualizarStatusConsulta(consultaPendente, 'EM_EXECUCAO', 'IniciarConsultaEvent')
}
...
function finalizarConsultaTransaction(consultaIniciada) {
    return atualizarStatusConsulta(consultaIniciada, 'FINALIZADA', 'FinalizarConsultaEvent')
}

async function atualizarStatusExame(exameTransaction, status, evento, updateDataResultado = false) {
    ...
    if (exameBlock.fornecedor.$identifier === exameTransaction.fornecedor.$identifier) {
        ...
        let exameEvent = factory.newEvent(NS, evento);
...
async function atualizarStatusConsulta(consultaTransaction, status, evento) {
    console.log(`consulta ${consultaTransaction.consulta.consultaId} ${status}`)
    let factory = getFactory();
    let assetRegistry = await getAssetRegistry('com.ita.stagiopbd.Consulta')
